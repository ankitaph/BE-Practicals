...
for i in range(2):
    ...
    top_antibodies.sort(key = get_key, reverse=True)  #descending order based on fitness value or affinity value
...
potential_damage_index = damaged_affinities.index(max(damaged_affinities))
    ...
    #returns antibody with highest affinity with damaged data
