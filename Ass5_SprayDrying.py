...
true_coefficients = (
    np.random.rand(num_features) * 10
)  # Random coefficients for features
...
number_of_parameters = (
    4  # Adjust based on the number of parameters you want to optimize
)
varbound = np.array(
    [
        [5, 50],  # Range for number of neurons in each hidden layer
        [1, 5],  # Range for number of hidden layers
        [
            0,
            3,
        ],  # Activation function: 0 for identity, 1 for logistic, 2 for tanh, 3 for relu
        [0, 2],
    ]
)  # Solver: 0 for lbfgs, 1 for sgd, 2 for adam
algorithm_param = {
    "max_num_iteration": 1,
    "population_size": 100,
...
model = ga(
    ...
    dimension=number_of_parameters,
    variable_type="int",
